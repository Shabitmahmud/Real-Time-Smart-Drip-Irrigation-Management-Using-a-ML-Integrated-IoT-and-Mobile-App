...
class _VolumeScreenState extends State<VolumeScreen> {
  ...
  void initState() {
    super.initState();
    fetchData('http://127.0.0.1:8000/volume_last_15_values/?field_id=f2'); // Fetch default data for field_id = 'f2'
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (dataList.isNotEmpty) {
          setState(() {
            volumeData = dataList
                .map<double>((data) => data['volume']?.toDouble() ?? 0.0)
                .toList();
            timestamps = dataList
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Volume Graph (Field 2)'),
        actions: [
          IconButton(
            ...
            onPressed: () {
              if (selectedCondition == "Last 15 Values") {
                fetchData(
                    'http://127.0.0.1:8000/volume_last_15_values/?field_id=f2');
              } else if (selectedCondition == "Last 7 Days Avg") {
                fetchData(
                    'http://127.0.0.1:8000/volume_last_7_days_avg/?field_id=f2');
              } else if (selectedCondition == "Last 15 Days Avg") {
                fetchData(
                    'http://127.0.0.1:8000/volume_last_15_days_avg/?field_id=f2');
              }
            },
          ),
        ],
                    ...
                    if (volumeData.isNotEmpty)
                      LineChartBarData(
                        spots: List.generate(
                          volumeData.length,
                              (index) =>
                              FlSpot(index.toDouble(), volumeData[index]),
                        ),
                        isCurved: true,
                        colors: [Colors.green],
                        barWidth: 2,
                        ...
                        return touchedSpots.map((LineBarSpot touchedSpot) {
                          ...
                          if (xIndex >= 0 && xIndex < timestamps.length) {
                            return LineTooltipItem(
                              'Timestamp: ${timestamps[xIndex]}, Volume: ${touchedSpot.y.toStringAsFixed(2)}',
                              TextStyle(
                                  color: Colors.white,
                                  fontWeight: FontWeight.bold),
                            );
