...
class FlowRateScreen extends StatefulWidget {
  @override
  _FlowRateScreenState createState() => _FlowRateScreenState();
}

class _FlowRateScreenState extends State<FlowRateScreen> {
  List<double> flowRateData = [];
  ...
  void initState() {
    super.initState();
    fetchData('http://127.0.0.1:8000/flow_rate_last_15_values/?field_id=f2'); // Fetch default data for field_id = 'f2'
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (dataList.isNotEmpty) {
          setState(() {
            flowRateData = dataList
                .map<double>((data) => data['flow_rate']?.toDouble() ?? 0.0)
                .toList();
            timestamps = dataList
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Flow Rate Graph (Field 2)'),
        actions: [
          IconButton(
            ...
            onPressed: () {
              if (selectedCondition == "Last 15 Values") {
                fetchData(
                    'http://127.0.0.1:8000/flow_rate_last_15_values/?field_id=f2');
              } else if (selectedCondition == "Last 7 Days Avg") {
                fetchData(
                    'http://127.0.0.1:8000/flow_rate_last_7_days_avg/?field_id=f2');
              } else if (selectedCondition == "Last 15 Days Avg") {
                fetchData(
                    'http://127.0.0.1:8000/flow_rate_last_15_days_avg/?field_id=f2');
              }
            },
          ),
        ],
          ...
          children: [
            ...
            Expanded(
              child: LineChart(
                LineChartData(
                  minX: 0,
                  maxX: flowRateData.length.toDouble() - 1,
                  minY: 0,
                  maxY: (flowRateData.isNotEmpty
                      ? flowRateData.reduce((a, b) => a > b ? a : b)
                      : 100) +
                      10,
                  ...
                  lineBarsData: [
                    if (flowRateData.isNotEmpty)
                      LineChartBarData(
                        spots: List.generate(
                          flowRateData.length,
                              (index) =>
                              FlSpot(index.toDouble(), flowRateData[index]),
                        ),
                        isCurved: true,
                        ...
                        return touchedSpots.map((LineBarSpot touchedSpot) {
                          ...
                          if (xIndex >= 0 && xIndex < timestamps.length) {
                            return LineTooltipItem(
                              'Timestamp: ${timestamps[xIndex]}, Flow Rate: ${touchedSpot.y.toStringAsFixed(2)}',
                              TextStyle(
                                  color: Colors.white,
                                  fontWeight: FontWeight.bold),
                            );
