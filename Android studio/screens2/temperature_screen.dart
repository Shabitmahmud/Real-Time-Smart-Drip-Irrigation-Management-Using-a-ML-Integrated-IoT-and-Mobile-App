...
class _TemperatureScreenState extends State<TemperatureScreen> {
  ...
  void initState() {
    super.initState();
    fetchData('http://127.0.0.1:8000/last_15_values/?field_id=f2'); // Fetch default data for field_id = 'f2'
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Temperature Graph (Field 2)'),
        actions: [
          IconButton(
            icon: Icon(Icons.refresh),
            onPressed: () {
              if (selectedCondition == "Last 15 Values") {
                fetchData('http://127.0.0.1:8000/last_15_values/?field_id=f2');
              } else if (selectedCondition == "Last 7 Days Avg") {
                fetchData('http://127.0.0.1:8000/last_7_days_avg/?field_id=f2');
              } else if (selectedCondition == "Last 15 Days Avg") {
                fetchData('http://127.0.0.1:8000/last_15_days_avg/?field_id=f2');
              }
            },
          ),
        ],
